...
export const registerExtension = function (this: Editor, extension: Extension) {
  ...
  if (extension.onDomParseNode) {
    const fn = this.onDomParseNode
    this.onDomParseNode = node => {
      node = extension.onDomParseNode!.apply(this, [node])
  ...
  if (extension.onPasteKeepMarks) {
    const fn = this.onPasteKeepMarks
    this.onPasteKeepMarks = node => {
      const marks = extension.onPasteKeepMarks!.apply(this, [node])
      if (fn) Object.assign(marks, fn.apply(this, [node]))
      return marks
    }
  }
  if (extension.onPasteKeepStyles) {
    const fn = this.onPasteKeepStyles
    this.onPasteKeepStyles = node => {
      const styles = extension.onPasteKeepStyles!.apply(this, [node])
      if (fn) Object.assign(styles, fn.apply(this, [node]))
      return styles
    }
  }
  if (extension.onBeforeUpdateView) {
    const fn = this.onBeforeUpdateView
    this.onBeforeUpdateView = () => {
      extension.onBeforeUpdateView!.apply(this)
      if (fn) fn.apply(this)
    }
  }
  if (extension.onAfterUpdateView) {
    const fn = this.onAfterUpdateView
    this.onAfterUpdateView = () => {
      extension.onAfterUpdateView!.apply(this)
      if (fn) fn.apply(this)
    }
  }
  if (extension.onDetachMentBlockFromParent) {
    const fn = this.onDetachMentBlockFromParent
    this.onDetachMentBlockFromParent = node => {
      const useDefault = extension.onDetachMentBlockFromParent!.apply(this, [node])
      if (fn) return fn.apply(this, [node]) && useDefault
      return useDefault
    }
  }
  if (extension.onBeforePatchNodeToFormat) {
    const fn = this.onBeforePatchNodeToFormat
    this.onBeforePatchNodeToFormat = node => {
      node = extension.onBeforePatchNodeToFormat!.apply(this, [node])
      if (fn) node = fn.apply(this, [node])
      return node
    }
  }
...
export const handlerForPasteKeepMarksAndStyles = function (this: Editor, nodes: KNode[]) {
  ...
  for (let i = 0; i < length; i++) {
    ...
    //自定义标记保留
    if (typeof this.onPasteKeepMarks == 'function') {
      const extendMarks = this.onPasteKeepMarks.apply(this, [node])
      Object.assign(marks, extendMarks)
    }
    //自定义样式保留
    if (typeof this.onPasteKeepStyles == 'function') {
      const extendStyles = this.onPasteKeepStyles.apply(this, [node])
...
export const handlerForPasteFiles = async function (this: Editor, files: FileList) {
  ...
  for (let i = 0; i < length; i++) {
    ...
    if (file.type.startsWith('image/')) {
      //是否走默认逻辑
      const useDefault = typeof this.onPasteImage == 'function' ? await this.onPasteImage.apply(this, [file]) : true
      //走默认逻辑
      if (useDefault) {
        const url = await DapFile.dataFileToBase64(file)
    ...
    //视频粘贴
      ...
      //是否走默认逻辑
      const useDefault = typeof this.onPasteVideo == 'function' ? await this.onPasteVideo.apply(this, [file]) : true
      //走默认逻辑
      if (useDefault) {
        const url = await DapFile.dataFileToBase64(file)
    ...
    //其他文件粘贴
      ...
      this.onPasteFile.apply(this, [file])
    }
  }
}
...
export const handlerForPasteDrop = async function (this: Editor, dataTransfer: DataTransfer) {
  ...
  //允许粘贴html，则处理html粘贴（光标在代码块内时降级为文本粘贴，避免html结构破坏代码格式）
...
export const setPlaceholder = function (this: Editor) {
  if (this.isEmpty() && !this.compositionData.isInput) {
