...
export const onComposition = async function (this: Editor, e: Event) {
  ...
  if (event.type == 'compositionstart') {
    //改变标识
    this.compositionData.isInput = true
    //记录编辑器内容
    this.compositionData.oldHtml = this.$el!.innerHTML
    //设置placeholder
  ...
  //输入中文结束后
    ...
    //改变标识
    this.compositionData.isInput = false
    //记录编辑器内容
    this.compositionData.newHtml = this.$el!.innerHTML
    //更新视图
    await this.updateView()
    //恢复中文输入的记录数据
    this.compositionData.oldHtml = undefined
    this.compositionData.newHtml = undefined
  }
}
 ...
 * 监听键盘事件
 */
export const onKeyboard = async function (this: Editor, e: Event) {
  if (this.compositionData.isInput) {
    return
  }
