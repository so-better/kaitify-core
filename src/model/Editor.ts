...
import { formatSiblingNodesMerge, formatPlaceholderMerge, formatZeroWidthTextMerge, RuleFunctionType, formatParentNodeMerge, formatBlockInChildren, formatBlockTagParse, formatLineBreakSpaceText, formatContenteditableToClosed } from './config/format-rules'
...
export class Editor {
   ...
   */
  formatRules: RuleFunctionType[] = [formatContenteditableToClosed, formatBlockTagParse, formatBlockInChildren, formatPlaceholderMerge, formatZeroWidthTextMerge, formatLineBreakSpaceText, formatSiblingNodesMerge, formatParentNodeMerge]
  /**
  ...
  findNode(dom: HTMLElement) {
    ...
    //从dom开始向上查找具有 NODE_MARK 标记的元素
    let el: HTMLElement | null = dom
    while (el && el !== this.$el) {
      const key = el.getAttribute(NODE_MARK)
      //存在标记
      if (key) {
        //查找到节点
    ...
    throw new Error(`Cannot find ${NODE_MARK} attribute for this dom`)
  }
   ...
   */
  getClosedNodeBySelection(options: KNodeMatchOptionType = {}) {
    ...
    if (startNode.isEqual(endNode) && startNode.isClosed() && startNode.isMatch(options) && startOffset == 0 && endOffset == 1) {
      return startNode
    }
    return null
  }
   ...
   */
  hasClosedNodeBySelection(options: KNodeMatchOptionType = {}) {
    if (!this.selection.focused() || this.selection.collapsed()) {
      return false
    }
    const startNode = this.selection.start!.node
    ...
    const endOffset = this.selection.end!.offset
    // 起点从闭合节点头部开始
    if (startNode.isClosed() && startNode.isMatch(options) && startOffset === 0) {
      return true
    }
    // 终点到闭合节点尾部结束
    if (endNode.isClosed() && endNode.isMatch(options) && endOffset === 1) {
      return true
    }
    // 选区中间完整包含的闭合节点（排除边界节点）
    return this.getFocusNodesBySelection('all')
      .filter(n => !n.isEqual(startNode) && !n.isEqual(endNode))
      .some(n => n.isClosed() && n.isMatch(options))
  }
  ...
  setDark(dark: boolean) {
    if (dark) {
      this.$el?.classList.add('kaitify-dark')
    } else {
      this.$el?.classList.remove('kaitify-dark')
    }
  }
   ...
   */
  isDark() {
    return this.$el?.classList.contains('kaitify-dark') ?? false
  }
  ...
  destroy() {
    ...
    if (this.$el) {
      ...
      //移除绑定在元素上的事件
      DapEvent.off(this.$el, 'beforeinput.kaitify compositionstart.kaitify compositionupdate.kaitify compositionend.kaitify keydown.kaitify keyup.kaitify copy.kaitify cut.kaitify focus.kaitify blur.kaitify')
    }
    //重置selection
