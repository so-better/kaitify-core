...
export class Editor {
	...
	getClosedNodeBySelection(options: KNodeMatchOptionType) {
		...
		if (startNode.isEqual(endNode) && startNode.isMatch(options) && startOffset == 0 && endOffset == 1) {
			return startNode
		}
		return null
	}
	...
	hasClosedNodeBySelection(options: KNodeMatchOptionType) {
		...
		const endOffset = this.selection.end!.offset
		// 起点从附件头部开始
		if (startNode.isMatch(options) && startOffset === 0) {
			return true
		}
		// 终点到附件尾部结束
		if (endNode.isMatch(options) && endOffset === 1) {
			return true
		}
		// 选区中间完整包含的附件（排除边界节点）
		return this.getFocusNodesBySelection('all')
			.filter(n => !n.isEqual(startNode) && !n.isEqual(endNode))
			.some(n => n.isMatch(options))
	}
