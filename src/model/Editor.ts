		...
		patchNodes(this.stackNodes, oldStackNodes).forEach(item => {
			...
			if (item.newNode) {
				this.formatRules.forEach(rule => {
					formatNodes.apply(this, [rule, item.newNode!.parent ? item.newNode!.parent.children! : this.stackNodes])
				})
			}
			//没有新节点但是有旧节点，表示remove，如果是根级块节点remove就不用处理，非根级块节点需要对新数组中的父节点的子数组进行格式化
				...
				if (parentNode) {
					this.formatRules.forEach(rule => {
						formatNodes.apply(this, [rule, parentNode.children!])
					})
				}
			}
		})
