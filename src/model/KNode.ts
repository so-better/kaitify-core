...
export class KNode {
  ...
  /**
   * 样式集合【可以修改】
   * 样式名称请使用驼峰写法，虽然在渲染时兼容处理了中划线格式的样式名称，但是在其他地方可能会出现问题并且编辑器内部在样式相关的判断都是以驼峰写法为主
   */
  styles?: KNodeStylesType
  /**
   * 是否锁定节点，锁定的节点都不允许合并【可以修改】：
  ...
  /**
   * 是否为固定块节点，值为true时：
   * - 当光标在节点起始处或者光标在节点内只有占位符时，执行删除操作不会删除此节点，会再次创建一个占位符进行处理；
   * - 当光标在节点内且节点不是代码块样式，不会进行换行【可以修改】
   */
  fixed: boolean = false
  /**
