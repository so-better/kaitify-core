...
- 详细信息
  ...
  提供一个入参，类型为 `KNode`，表示指定的节点，该方法会在该节点内查找最后一个可以设置为光标的节点，如果该节点本身就是文本节点或者闭合节点，则直接返回自身，如果该节点是空节点或者不可见节点，则返回 `null`
...
- 详细信息
  ...
  提供一个入参，类型为 `KNode`，表示指定的节点，该方法会在该节点内查找第一个可以设置为光标的节点，如果该节点本身就是文本节点或者闭合节点，则直接返回自身，如果该节点是空节点或者不可见节点，则返回 `null`
...
如果虚拟光标是折叠状态，执行此方法，相当于按了一次退格键（Backspace），会删除光标之前的一个字符或者一个图片等；如果虚拟光标不是折叠状态，则会对光标选中的范围内的节点进行删除
...
- 详细信息
  ...
  销毁编辑器。该方法会将编辑器所在的容器恢复成之前的模样，不再具有可编辑的效果，并且移除相关监听事件
