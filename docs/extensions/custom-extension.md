...
> 注意：定义扩展时避免出现块节点嵌套的结构，这样在渲染时可能会出现很多意想不到的问题；如果真的有需要，必须结合nested和fixed等属性进行周密的设置，因为块节点需要考虑getBlock、toParagraph等语法和换行、删除等操作带来的影响。推荐使用block+inline或者closed这些设计方式，而不使用block+block的设计方式

> 自定义扩展时，可灵活地运用KNode的locked、void、fixed、nested等属性，使得编辑器在格式化和渲染过程中，能够符合预期地进行
