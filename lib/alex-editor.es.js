...
const getElementByKey = function(key, stack) {
  if (!key) {
    return null;
  }
  const fn = (elements) => {
...
const _AlexElement = class _AlexElement {
  constructor(type, parsedom, marks, styles, textContent) {
     ...
     */
    __publicField(this, "forceUpdate", false);
    /**
     * 真实node
     */
  ...
  clone(deep = true) {
    ...
    let el = new _AlexElement(this.type, this.parsedom, common.clone(this.marks), common.clone(this.styles), this.textContent);
  ...
  isFirst(element2) {
    ...
    if (element2.isContains(this)) {
      const elements = _AlexElement.flatElements(element2.children).filter((el) => el.isText() || el.isClosed());
  ...
  __render() {
    ...
    if (this.isText()) {
      if (this.namespace) {
        el = document.createElementNS(this.namespace, _AlexElement.TEXT_NODE);
      } else {
  ...
  static create(elementConfig) {
    ...
    if (elementConfig.type == "text") {
      el = new _AlexElement(elementConfig.type, null, elementConfig.marks ? elementConfig.marks : null, elementConfig.styles ? elementConfig.styles : null, elementConfig.textcontent ? elementConfig.textcontent : null);
    } else {
      el = new _AlexElement(elementConfig.type, elementConfig.parsedom, elementConfig.marks ? elementConfig.marks : null, elementConfig.styles ? elementConfig.styles : null, null);
      ...
      if (elementConfig.type != "closed" && Array.isArray(elementConfig.children)) {
        el.children = elementConfig.children.map((item) => {
          const child = _AlexElement.create(item);
          child.parent = el;
          return child;
        });
      }
...
const breakFormat = function(element2) {
  if (element2.hasChildren()) {
    ...
    if (element2.isInline() && breaks.length) {
      breaks.forEach((el) => {
        el.toEmpty();
      });
...
const mergeWithParentElement = function(element2) {
  ...
  const merge = (parent, child) => {
    if (child.isText()) {
      ...
      if (child.hasMarks()) {
        if (parent.hasMarks()) {
          Object.assign(parent.marks, common.clone(child.marks));
        } else {
          parent.marks = common.clone(child.marks);
        }
      }
      if (child.hasStyles()) {
        if (parent.hasStyles()) {
          Object.assign(parent.styles, common.clone(child.styles));
        } else {
          parent.styles = common.clone(child.styles);
        }
      }
...
const mergeWithSpaceTextElement = function(element2) {
  if (element2.isText() && !element2.isEmpty() && element2.textContent.split("").some((item) => isSpaceText(item))) {
      ...
      if (mutationList[i].type == "childList") {
        ...
        for (let j = 0; j < addNodesLength; j++) {
          ...
          if (node.nodeType == 3) {
            ...
            const key = parentNode ? data.get(parentNode, "data-alex-editor-key") : null;
            const element2 = key ? this.getElementByKey(key) : null;
            if (element2 && !element2.isText()) {
              this.__illegalDoms.push(node);
            }
          } else if (element.isElement(node) && !data.get(node, "data-alex-editor-key")) {
            this.__illegalDoms.push(node);
          }
        }
      }
...
const removeEditorDomObserve = function() {
  if (this.__domObserver) {
    this.__domObserver.disconnect();
    this.__domObserver = null;
  }
...
const formatElement = function(element2, fn, receiver) {
  const index = receiver.findIndex((el) => el.isEqual(element2));
  if (index < 0) {
    return;
  }
  if (element2.isEmpty()) {
    ...
    if (element2.isEmpty()) {
      ...
      const findIndex = receiver.findIndex((el) => el.isEqual(element2));
      receiver.splice(findIndex, 1);
    } else {
      if (!element2.isBlock() && receiver === this.stack) {
        element2.convertToBlock();
      }
      if (element2.hasChildren()) {
        element2.children.forEach((child) => {
          formatElement.apply(this, [child, fn, element2.children]);
        });
      }
      if (element2.isEmpty()) {
        if (this.range && element2.isContains(this.range.anchor.element)) {
...
const patch = (newElements, oldElements, forRender) => {
  let result = [];
...
const patchElement = (newElement, oldElement, forRender) => {
  const result = [];
...
const getDifferentStyles = (newElement, oldElement) => {
  const setStyles = {};
  const removeStyles = {};
...
const getDifferentMarks = (newElement, oldElement) => {
  const setMarks = {};
  const removeMarks = {};
...
class AlexEditor {
  constructor(node, opts) {
    ...
    event.on(this.$el, "keydown.alex_editor keyup.alex_editor", handleKeyboard.bind(this));
    ...
    if (firstRender) {
      ...
      patch(this.stack, this.__oldStack, true).forEach((item) => {
        ...
        if (item.type == "insert") {
          ...
          (_b = (_a = item.oldElement) == null ? void 0 : _a.elm) == null ? void 0 : _b.remove();
