...
class AlexPoint {
  ...
  moveToEnd(element2) {
    ...
    if (element2.getUneditableElement()) {
      throw new Error("Uneditable element cannot be set as focal point");
    }
    if (element2.isText()) {
      this.element = element2;
...
const handleUneditableBlock = function(element2) {
  if (element2.isBlock() && element2.getUneditableElement()) {
    let marks = [];
  ...
  if (element2.isBlock() && element2.hasChildren()) {
    const isAllUneditable = AlexElement.flatElements(element2.children).every((el) => {
      return !!el.getUneditableElement();
    });
    if (isAllUneditable) {
      this.addElementTo(AlexElement.getSpaceElement(), element2, element2.children.length);
      this.addElementTo(AlexElement.getSpaceElement(), element2, 0);
    }
  }
};
