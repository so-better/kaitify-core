    ...
    if (block2) {
      ...
      if (inline2.parse) {
        ...
        if (obj.common.isObject(inline2.parse)) {
          for (let key in inline2.parse) {
            if (typeof inline2.parse[key] == "function") {
              config.styles[key] = inline2.parse[key].apply(this, [node]);
            } else {
              config.styles[key] = inline2.parse[key];
            }
          }
        }
      }
