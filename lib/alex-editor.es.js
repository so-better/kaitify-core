...
const isContains = function(parentNode, childNode) {
  if (childNode.nodeType == 3) {
    return obj.element.isContains(parentNode, childNode.parentNode);
  }
  return obj.element.isContains(parentNode, childNode);
};
...
const initEditorNode = function(node) {
  ...
  if (obj.data.get(node, "data-alex-editor-init")) {
    throw new Error("The element node has been initialized to the editor");
  }
  obj.data.set(node, "data-alex-editor-init", true);
  return node;
};
...
const queryHasValue = function(obj$1, name, value) {
  if (value == null || value == void 0) {
    return obj$1.hasOwnProperty(name);
  }
  let ownValue = obj$1[name];
  ...
  if (typeof value == "string" && value && (obj.common.matchingText(value, "rgb") || obj.common.matchingText(value, "rgba"))) {
    value = obj.string.trim(value, true);
  }
  if (typeof ownValue == "string" && ownValue && (obj.common.matchingText(ownValue, "rgb") || obj.common.matchingText(ownValue, "rgba"))) {
    ownValue = obj.string.trim(ownValue, true);
  }
  if (typeof value == "string" && value && obj.common.matchingText(value, "hex")) {
    const arr = obj.color.hex2rgb(value);
    value = `rgb(${arr[0]},${arr[1]},${arr[2]})`;
  }
  if (typeof ownValue == "string" && ownValue && obj.common.matchingText(ownValue, "hex")) {
    const arr = obj.color.hex2rgb(ownValue);
...
const setRangeInVisible = function() {
  var _a;
  const fn = async (root) => {
    const scrollHeight = obj.element.getScrollHeight(root);
    const scrollWidth = obj.element.getScrollWidth(root);
    if (root.clientHeight < scrollHeight || root.clientWidth < scrollWidth) {
      ...
      if (root.clientWidth < scrollWidth) {
        if (childRect.right < parentRect.left) {
          await obj.element.setScrollLeft({
            el: root,
            number: 0
          });
          const tempChildRect = target.getBoundingClientRect();
          const tempParentRect = root.getBoundingClientRect();
          obj.element.setScrollLeft({
            el: root,
...
class AlexEditor {
  constructor(node, opts) {
    ...
    obj.event.on(document, `selectionchange.alex_editor_${this.__guid}`, handleSelectionChange.bind(this));
    obj.event.on(this.$el, "beforeinput.alex_editor", handleBeforeInput.bind(this));
    obj.event.on(this.$el, "compositionstart.alex_editor compositionupdate.alex_editor compositionend.alex_editor", handleChineseInput.bind(this));
    obj.event.on(this.$el, "keydown.alex_editor", handleKeydown.bind(this));
    obj.event.on(this.$el, "cut.alex_editor", handleCut.bind(this));
    obj.event.on(this.$el, "paste.alex_editor", handlePaste.bind(this));
    obj.event.on(this.$el, "copy.alex_editor", handleCopy.bind(this));
    obj.event.on(this.$el, "dragstart.alex_editor drop.alex_editor ", handleDragDrop.bind(this));
    obj.event.on(this.$el, "focus.alex_editor", handleFocus.bind(this));
    obj.event.on(this.$el, "blur.alex_editor", handleBlur.bind(this));
  }
  /**
   * 初始化range
   */
        ...
        if (blob.type == "text/plain" && !this.allowPasteHtml) {
          ...
          if (data2) {
            if (typeof this.customTextPaste == "function") {
              await this.customTextPaste.apply(this, [data2, useCache]);
            } else {
              this.insertText(data2, useCache);
          ...
          if (data2) {
            ...
            if (typeof this.customHtmlPaste == "function") {
              await this.customHtmlPaste.apply(this, [elements, data2, useCache]);
            } else {
              for (let i2 = 0; i2 < elements.length; i2++) {
                this.formatElement(elements[i2]);
                this.insertElement(elements[i2], false, i2 == 0 ? useCache : false);
              }
  ...
  parseNode(node) {
    ...
    if (block) {
      ...
      if (inline.parse) {
        ...
        if (obj.common.isObject(inline.parse)) {
