...
const _AlexElement = class {
  ...
  static flatElements(elements) {
    const flat = (arr) => {
      ...
      for (let i = 0; i < length; i++) {
        if (_AlexElement.isElement(arr[i])) {
          result.push(arr[i]);
  ...
  __setRangeInVisible() {
    ...
    const fn = (root2) => {
      ...
      if (root2.clientHeight < scrollHeight) {
        ...
        const childRect = obj.element.getElementBounding(el);
        const parentRect = obj.element.getElementBounding(root2);
        if (isHiddenTop(childRect.top, parentRect.top) || isHiddenBottom(childRect.bottom, parentRect.bottom)) {
          obj.element.setScrollTop({
            el: root2,
            number: 0
          }).then(() => {
            const tempChildRect = obj.element.getElementBounding(el);
            const tempParentRect = obj.element.getElementBounding(root2);
            obj.element.setScrollTop({
              el: root2,
  ...
  __updateChildren(oldElements, newElements) {
    ...
    for (let i = 0; i < length; i++) {
      ...
      if (diffMarks.less.length) {
        const length2 = diffMarks.less.length;
        for (let i2 = 0; i2 < length2; i2++) {
          newElement._elm.removeAttribute(diffMarks.less[i2]);
        }
      }
      if (diffMarks.more.length) {
        const length2 = diffMarks.more.length;
        for (let i2 = 0; i2 < length2; i2++) {
          newElement._elm.setAttribute(diffMarks.more[i2], newElement.marks[diffMarks.more[i2]]);
        }
      }
      const diffStyles = newElement.__getDiffStyles(oldElement);
      if (diffStyles.less.length) {
        const length2 = diffStyles.less.length;
        for (let i2 = 0; i2 < length2; i2++) {
          newElement._elm.style.setProperty(diffStyles.less[i2], "");
        }
      }
      if (diffStyles.more.length) {
        const length2 = diffStyles.more.length;
        for (let i2 = 0; i2 < length2; i2++) {
          newElement._elm.style.setProperty(diffStyles.more[i2], newElement.styles[diffStyles.more[i2]]);
        }
      }
