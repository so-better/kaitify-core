...
const queryHasValue = (obj$1, name, value) => {
  ...
  arr.forEach((item, index) => {
    if (item.offset) {
      ...
      if (item.offset[0] == 0) {
        ...
        globalThis.addElementAfter(el, item.element);
        ...
        globalThis.addElementAfter(el, item.element);
        globalThis.addElementAfter(el2, el);
        selectEl = el;
      }
      if (index == 0) {
        globalThis.range.anchor.moveToStart(selectEl);
      }
      if (index == arr.length - 1) {
        globalThis.range.focus.moveToEnd(selectEl);
      }
