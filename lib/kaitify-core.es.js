...
const registerExtension = function(extension) {
  ...
  if (extension.pasteKeepStyles) {
    ...
    this.pasteKeepStyles = (node) => {
      ...
      if (fn)
        Object.assign(styles, fn.apply(this, [node]));
      return styles;
    };
  }
...
class Extension {
  ...
  static create(options) {
    ...
    if (options.formatRule)
      extension.formatRule = options.formatRule;
...
const BackColorExtension = Extension.create({
  ...
  addCommands() {
    ...
    const unsetBackColor = async (val) => {
      if (!isBackColor(val)) {
        return;
      }
...
const clearAlign = (blockNode, val) => {
...
const AlignExtension = Extension.create({
  ...
  addCommands() {
    ...
    const setAlign = async (val) => {
      if (isAlign(val)) {
        return;
      }
      if (this.selection.collapsed()) {
        const blockNode = this.selection.start.node.getBlock();
        const styles = blockNode.hasStyles() ? blockNode.styles : {};
        blockNode.styles = {
          ...styles,
          textAlign: val
        };
        ...
        const blockNodes = getSelectionBlockNodes.apply(this);
        blockNodes.forEach((item) => {
          const styles = item.hasStyles() ? item.styles : {};
          item.styles = {
            ...styles,
            textAlign: val
          };
...
const clearLineHeight = (blockNode, val) => {
...
const LineHeightExtension = Extension.create({
  ...
  addCommands() {
    ...
    const unsetLineHeight = async (val) => {
      if (!isLineHeight(val)) {
        return;
      }
      if (this.selection.collapsed()) {
...
const clearIndent = (blockNode, val) => {
...
const insertDom = (editor, newNode) => {
...
const replaceDom = (editor, newNode, oldNode) => {
  const oldDom = findDom(editor, oldNode);
...
class Editor {
  constructor() {
     ...
     */
    __publicField(this, "formatRules", [formatBlockInChildren, formatUneditableNoodes, formatPlaceholderMerge, formatZeroWidthTextMerge, formatSiblingNodesMerge, formatParentNodeMerge]);
    /**
