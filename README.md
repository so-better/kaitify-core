...
| cut             | 进行剪切操作时触发                                                                                                                                                                                                                                                                                                                                                            |
| paste           | 进行粘贴操作时触发                                                                                                                                                                                                                                                                                                                                                            |
| deleteExtend    | 某些特定情形下的删除操作会触发此事件，回调参数 type 是一个数值，表示情景类型。如果 type 为 1 表示光标在编辑器的开始处，但是此刻所在的根级块元素或者内部块元素不为空；如果 type 为 2 表示光标在编辑器的开始处，但是此刻所在的根级块元素或者内部块元素为空。如果没有监听该事件触发，在 type 为 2 的情景下，会默认创建换行符。如果监听了该事件，type 为 2 的情景下则需要手动处理 |
